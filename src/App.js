...
const App = () => {
  ...
  return (
    <div className={"swipe-design"}>
        ...
        {posts.map((item, i)=>(
            <div key={item.id} className={`card  ${i < index ? 'none' : 'back'}`}
                 style={{zIndex:posts.length-i,
                     left: window.innerWidth > 376 ? `${655 - 13 * i}px` : `${143.4 - 8.94 * i}px`,
                     top: window.innerWidth > 376 ?`${ 385 - 13 * i}px` : `${324.76 - 8.94 * i}px`}}
        ...
        {/* Optional Reset Button at the end

        <button className={"reset"} onClick={()=>setIndex(0)}
        style={{left: window.innerWidth > 376 ? `${655 - 13 * (posts.length)}px` : `${143.4 - 8.94 * (posts.length)}px`,
            top: window.innerWidth > 376 ? `${385 - 13 * (posts.length)}px` : `${324.76 - 8.94 * (posts.length)}px`}}>
            Out of cards
